...
class FilterResult:
    ...
    def __init__(self, action: FilterAction, value: Optional[float] = None,
                 flag: Optional[str] = None, confidence: float = 1.0,
...
class QualityFilter(StreamFilter):
    ...
    def __init__(self, name: str = "quality_filter", 
                 ...
                 max_rate_change: Optional[float] = None,
                 enabled: bool = True):
...
class ValueFilter(StreamFilter):
    ...
    def __init__(self, name: str = "value_filter",
                 ...
                 valid_ranges: Optional[List[tuple[float, float]]] = None,
                 threshold_alerts: Optional[Dict[str, float]] = None,
                 max_change: Optional[float] = None,
                 validation_func: Optional[Callable[[float], bool]] = None,
                 enabled: bool = True):
        super().__init__(name, enabled)
...
class ConditionalFilter(StreamFilter):
    ...
    def _compile_condition(self):
        ...
        try:
            ...
            logger.error("conditional_filter_compile_error", 
    ...
    def apply(self, timestamp: float, value: float, 
        ...
        try:
            ...
            return FilterResult(
                FilterAction.FLAG,
                flag=f"Condition evaluation error: {str(e)}"
            )
...
class FilterChain:
    ...
    def process_batch(self, timestamps: np.ndarray, values: np.ndarray,
                     context: Optional[Dict[str, Any]] = None) -> tuple[np.ndarray, np.ndarray, List[str]]:
    ...
    def get_filter_by_name(self, name: str) -> Optional[StreamFilter]:
        """Get filter by name"""
        for filter_instance in self.filters:
