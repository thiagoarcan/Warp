...
from platform_base.core.models import DownsampleResult, ResultMetadata, QualityMetrics
...
def _lttb_downsample(
    ...
    for i in range(n_points - 2):
        ...
        if i < n_points - 3:  # Not the last bucket
            ...
            if next_bucket_end >= n_data:
                next_bucket_end = n_data
        ...
        if NUMBA_AVAILABLE and _lttb_bucket_numba is not None and len(t_bucket) > 10:
            ...
            for j, (t_val, v_val) in enumerate(zip(t_bucket, v_bucket)):
...
def downsample(
    ...
    try:
        ...
        raise DownsampleError(f"Downsampling failed: {str(e)}", {"method": method}) from e
