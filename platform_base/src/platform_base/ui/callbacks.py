...
from platform_base.processing.interpolation import interpolate, SUPPORTED_METHODS as INTERP_METHODS
from platform_base.processing.synchronization import synchronize, SUPPORTED_SYNC_METHODS
from platform_base.processing.calculus import derivative, integral
from platform_base.core.models import ViewData, TimeWindow
from platform_base.ui.export import (
    export_selection, export_session, ExportConfig, ExportFormat
)
from platform_base.ui.state import Selection, get_app_state
from platform_base.viz.streaming import StreamingEngine, StreamingState, StreamFilters
...
def _create_timeseries_figure(
    ...
    interp_mask: Optional[dict[str, np.ndarray]] = None
) -> go.Figure:
...
def register_callbacks(app, app_state):
    ...
    def _on_upload(contents, filename):
        ...
        try:
            ...
            return no_update, no_update, f"Upload failed: {str(e)}"
    ...
    def _update_series_and_time(dataset_id):
        ...
        try:
            ...
            series_options = [
                ...
                for name in dataset.series.keys()
            ]
            ...
            logger.error("update_series_failed", error=str(e))
            return [], 0, 100, [0, 100], {}
    ...
    def _update_plot(
        ...
        try:
            ...
            if sync_method and sync_method != "none" and len(series_data) > 1:
                try:
                    t_dict = {name: t_filtered for name in series_data}
            ...
            logger.error("plot_update_failed", error=str(e))
            return _create_empty_figure(f"Error: {str(e)}")
    ...
    def _streaming_tick(n_intervals, state, dataset_id):
        ...
        try:
            ...
            logger.error("streaming_tick_failed", error=str(e))
            raise PreventUpdate
    ...
    def _handle_export_data(csv_clicks, xlsx_clicks, parquet_clicks,
        ...
        try:
            ...
            logger.error("export_failed", format=export_format, error=str(e))
    ...
    def _handle_export_session(n_clicks):
        ...
        try:
            ...
            return f"Session export failed: {str(e)}"
    ...
    def _update_results_content(active_tab, dataset_id, selected_series, time_range):
        ...
        if active_tab == "stats" and dataset_id and selected_series:
            try:
                ...
                return html.P(f"Error computing statistics: {str(e)}")
