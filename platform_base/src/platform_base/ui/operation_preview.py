...
from platform_base.processing.interpolation import InterpolationMethod
from platform_base.processing.synchronization import SynchronizationMethod
from platform_base.processing.calculus import CalculusMethod
...
class PreviewProcessor(QObject):
    ...
    def submit_preview(self, request: PreviewRequest) -> Future:
        ...
        future = self.executor.submit(self._process_preview, request)
        return future
    ...
    def _process_preview(self, request: PreviewRequest) -> PreviewResult:
        ...
        try:
            ...
            logger.error("preview_processing_failed", 
                        request_id=request.request_id,
...
class PreviewVisualizationWidget(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        self.current_result: Optional[PreviewResult] = None
        self._setup_ui()
    ...
    def _update_comparison_plot(self):
        ...
        series_names = [k for k in result.original_data.keys() if not k.startswith('time')]
    ...
    def _update_statistics(self):
        ...
        # Data statistics
        for series_name in result.original_data.keys():
            if series_name.startswith('time'):
                continue
...
class OperationPreviewManager(QObject):
    ...
    def __init__(self, parent: Optional[QObject] = None):
        ...
        self.pending_request: Optional[PreviewRequest] = None
    ...
    def request_preview(self, operation_type: str, parameters: Dict[str, Any], 
                       input_data: Dict[str, np.ndarray]) -> Optional[str]:
...
_preview_manager: Optional[OperationPreviewManager] = None
