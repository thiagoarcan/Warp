...
class DataChunk:
    ...
    decimated_data: Optional[np.ndarray] = None
    loaded: bool = False
    loading: bool = False
...
class StreamingDataManager(QObject):
    ...
    def _load_chunk(self, chunk_idx: int):
        ...
        try:
            ...
            logger.error(f"chunk_load_error: idx={chunk_idx}, error={e}")
        finally:
            chunk.loading = False
    ...
    def get_overview(self, target_points: int = 1000) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for chunk_idx, chunk in sorted(self._chunks.items()):
...
# Singleton instance
_performance_renderer: Optional[PerformanceRenderer] = None
