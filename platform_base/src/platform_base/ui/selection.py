...
from platform_base.core.models import Dataset, ViewData, TimeWindow
...
class SelectionCriteria:
    ...
    selected_ranges: Optional[List[tuple]] = None
    
    # Seleção condicional
    series_id: Optional[str] = None
    condition: Optional[str] = None  # String de condição (ex: "> 10", "< mean + 2*std")
    predicate: Optional[Callable] = None  # Função de predicado customizada
...
class DataSelector:
    ...
    def __init__(self):
        self.active_selection: Optional[Selection] = None
        self.selection_history: List[Selection] = []
        
    def select_temporal(self, dataset: Dataset, start_seconds: float, 
                       end_seconds: float, series_ids: Optional[List[str]] = None) -> Selection:
        """
    ...
    def select_interactive(self, dataset: Dataset, selected_points: List[int],
                          series_ids: Optional[List[str]] = None) -> Selection:
        """
    ...
    def select_conditional(self, dataset: Dataset, series_id: str, 
                          condition: str, series_ids: Optional[List[str]] = None) -> Selection:
        """
        ...
        try:
            ...
            logger.error("conditional_selection_failed", 
    ...
    def select_by_predicate(self, dataset: Dataset, predicate: Callable[[np.ndarray], np.ndarray],
        ...
        try:
            ...
            logger.error("predicate_selection_failed", error=str(e))
