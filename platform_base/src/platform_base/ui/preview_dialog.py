...
class OperationPreviewDialog(QDialog):
    ...
    def _compute_preview(self):
        ...
        try:
            ...
            logger.error(f"Preview computation error: {e}")
            self._stats_content.setText(f"Erro: {str(e)}")
    ...
    def _preview_remove_outliers(self) -> np.ndarray:
        ...
        result = np.interp(np.arange(len(result)), 
...
def show_preview_dialog(operation_name: str, params: Dict[str, Any], 
                        series_data: np.ndarray, parent=None) -> Optional[np.ndarray]:
    """
