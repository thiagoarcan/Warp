...
class SyncState:
    ...
    master_view_id: ViewID | None = None
...
class MultiViewSynchronizer(QObject):
    ...
    def register_view(self, view_id: ViewID, widget: QWidget,
                     streaming_engine: StreamingEngine | None = None) -> None:
        """
    ...
    def unregister_view(self, view_id: ViewID) -> None:
        ...
        if self.sync_state.master_view_id == view_id:
            remaining_views = [vid for vid in self.views if vid != view_id]
            if remaining_views:
        ...
        for view_id, view_info in self.views.items():
            if (view_id != self.sync_state.master_view_id and
                ...
                try:
                    ...
                    view_info.streaming_engine._current_update()
                    ...
                    logger.exception("view_sync_failed", view_id=view_id, error=str(e))
            ...
            if view_id != source_view_id and view_info.sync_enabled:
                if hasattr(view_info.widget, "on_sync_update"):
                    try:
                        ...
                        logger.exception("broadcast_update_failed",
                                   source_view=source_view_id,
                                   target_view=view_id,
                                   error=str(e))
...
class ViewSyncMixin:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._view_id: ViewID | None = None
        self._synchronizer: MultiViewSynchronizer | None = None
...
_global_synchronizer: MultiViewSynchronizer | None = None
