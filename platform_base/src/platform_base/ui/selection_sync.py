...
from platform_base.ui.selection import Selection, SelectionMode, SelectionCriteria
from platform_base.ui.multi_view_sync import MultiViewSynchronizer, ViewSyncMixin
...
class SelectionSyncFilter:
    ...
    allowed_view_types: Optional[Set[str]] = None      # Tipos de view permitidos
    allowed_selection_modes: Optional[Set[SelectionMode]] = None  # Modos de seleção permitidos
    min_selection_size: int = 1                        # Tamanho mínimo de seleção
    max_selection_size: Optional[int] = None           # Tamanho máximo de seleção
...
class SelectionSynchronizer(QObject):
    ...
    def sync_selection_to_views(self, source_view_id: ViewID, selection: Selection, 
                               target_view_ids: Optional[List[ViewID]] = None) -> List[SelectionSyncEvent]:
        """
        ...
        if target_view_ids is None:
            target_view_ids = [vid for vid in self.registered_views.keys() if vid != source_view_id]
    ...
    @pyqtSlot(str, object)
    def _on_view_selection_changed(self, view_id: ViewID, selection: Optional[Selection]):
        ...
        for view_id, view in self.registered_views.items():
            if view_id != source_view_id:
                try:
                    ...
                    logger.error("clear_selection_failed", 
    ...
    def _should_sync_to_view(self, source_view_id: ViewID, target_view_id: ViewID, 
        ...
        target_view = self.registered_views[target_view_id]
        if (filter_config.allowed_view_types and 
            hasattr(target_view, 'view_type') and
            target_view.view_type not in filter_config.allowed_view_types):
            return False
        
        return True
    
    def _transform_selection_for_view(self, selection: Selection, 
                                    target_view_id: ViewID) -> Optional[Selection]:
        ...
        if hasattr(target_view, 'transform_synced_selection'):
            try:
                ...
                logger.error("selection_transformation_failed", 
...
class SelectionSyncView:
    ...
    def transform_synced_selection(self, selection: Selection) -> Optional[Selection]:
        """
    ...
    def emit_selection_changed(self, selection: Optional[Selection]):
...
class GlobalSelectionSynchronizer(QObject):
    ...
    def __init__(self, multi_view_sync: Optional[MultiViewSynchronizer] = None):
        super().__init__()
...
# Global instance
_global_selection_sync: Optional[GlobalSelectionSynchronizer] = None


def get_global_selection_synchronizer(multi_view_sync: Optional[MultiViewSynchronizer] = None) -> GlobalSelectionSynchronizer:
...
def sync_temporal_selection_across_views(synchronizer: SelectionSynchronizer,
    ...
    from platform_base.ui.selection import SelectionCriteria
...
def create_selection_sync_filter(view_types: Optional[List[str]] = None,
                                selection_modes: Optional[List[SelectionMode]] = None,
                                min_size: int = 1,
                                max_size: Optional[int] = None) -> SelectionSyncFilter:
