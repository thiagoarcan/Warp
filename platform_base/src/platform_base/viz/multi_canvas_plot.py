...
class IndependentPlotCanvas(pg.PlotWidget):
    ...
    def _find_series_at_position(self, scene_pos) -> str | None:
    ...
    def add_series_to_canvas(self, series_id: str, x_data: np.ndarray, y_data: np.ndarray,
                            series_name: str | None = None, axis_index: int = 0):
        """
        ...
        if axis_index == 0:
            self.addItem(curve)
        elif axis_index - 1 < len(self._axis_viewboxes):
...
class MultiCanvasPlotWidget(QWidget):
    ...
    def _update_grid_layout(self):
        ...
        if num_canvases == 1:
            ...
            _rows, cols = 2, 2  # 3 canvases in 2x2 grid
        else:  # 4 canvases
            _rows, cols = 2, 2
    ...
    def get_canvas(self, canvas_id: int) -> IndependentPlotCanvas | None:
