...
class ConfigManager:
    ...
    def _load_source(self, source: ConfigSource):
        ...
        try:
            ...
            # Calculate checksum
            new_checksum = hashlib.md5(content.encode(), usedforsecurity=False).hexdigest()
            ...
            logger.exception("config_source_load_failed",
    ...
    def _notify_changes(self, old_config: dict[str, Any], new_config: dict[str, Any], source_name: str | None = None):
        ...
        if changed_keys:
            ...
            for callback in self.change_callbacks:
                try:
                    ...
                    logger.exception("config_callback_failed", error=str(e))
    ...
    def get(self, key: str, default: Any = None, validate_schema: str | None = None) -> Any:
    ...
    def keys(self, prefix: str = "") -> list[str]:
        ...
        with self._lock:
            ...
            return [k for k in self.config_data if k.startswith(prefix)]
        ...
        with self._lock:
            ...
            for source in self.sources:
                try:
                    ...
                    logger.exception("config_source_reload_failed",
...
def get_config(key: str, default: Any = None, validate_schema: str | None = None) -> Any:
