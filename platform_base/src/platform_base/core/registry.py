...
class VersionCompatibility:
    ...
    def is_compatible_api_version(plugin_api_version: str,
        ...
        return not (max_version and VersionCompatibility.compare_versions(plugin_api_version, max_version) > 0)
...
class PluginManifest:
    ...
    min_compatible_version: str | None = None  # Minimum compatible plugin API version
    max_compatible_version: str | None = None  # Maximum compatible plugin API version
...
class AdvancedPluginSandbox:
    ...
    def _resource_monitor_loop(self):
        ...
        try:
            ...
            logger.exception("resource_monitor_failed", plugin=self.plugin_name, error=str(e))
    ...
    def _report_violation(self, violation: SecurityViolation):
        ...
        for callback in self._violation_callbacks:
            try:
                ...
                logger.exception("violation_callback_failed", error=str(e))
...
class PluginRegistry:
        ...
        with self._discovery_lock:
            ...
            for directory in self._plugin_directories:
                try:
                    ...
                    logger.exception("plugin_discovery_failed", directory=str(directory), error=str(e))
            ...
            try:
                ...
                if auto_load and manifest.trusted:
                    try:
                        ...
                        logger.exception("plugin_autoload_failed",
                                   name=manifest.name, error=str(e))

            except Exception as e:
                logger.exception("plugin_manifest_load_failed",
    ...
    def load_plugin(self, name: str) -> PluginProtocol:
        ...
        try:
            ...
            logger.exception("plugin_load_failed", name=name, error=str(e))
    ...
    def safe_call(self, name: str, method: str, *args, timeout: float | None = None, **kwargs):
        """
        ...
        try:
            ...
            logger.exception("plugin_call_failed", name=name, method=method, error=str(exc))
    ...
    def _quarantine_plugin(self, plugin_name: str, reason: str):
        ...
        if plugin_name in self._plugins:
            try:
                ...
                logger.exception("quarantine_unload_failed", plugin=plugin_name, error=str(e))
    ...
    def _check_plugin_health(self):
        ...
        for plugin_name, plugin_info in self._plugins.items():
            ...
            try:
                ...
                logger.exception("plugin_health_check_failed",
    ...
    def get_sandbox_statistics(self) -> dict[str, Any]:
        ...
        for plugin_name, sandbox in self._sandboxes.items():
            try:
                ...
                logger.exception("sandbox_stats_failed", plugin=plugin_name, error=str(e))
        ...
        try:
            ...
            if was_loaded:
                try:
                    ...
                    logger.exception("plugin_upgrade_reload_failed",
                               plugin=plugin_name, error=str(e))
                    return False
            ...
            logger.exception("plugin_upgrade_failed",
                        plugin=plugin_name, error=str(e))
            return False
