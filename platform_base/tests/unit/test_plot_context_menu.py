...
class TestMathAnalysisDialog:
    ...
    def test_dialog_creation_interpolate(self, qtbot):
        ...
        # interpolate operation uses the shared dialog - no specific widgets required
    ...
    def test_dialog_creation_resample(self, qtbot):
        ...
        # resample operation uses the shared dialog - no specific widgets required
        assert hasattr(dialog, "apply_btn")
    ...
    def test_derivative_order_range(self, qtbot):
        ...
        assert dialog.derivative_order.maximum() == 5  # UI defines max=5
    ...
    def test_derivative_methods_available(self, qtbot):
        ...
        # Verify methods defined in the UI file
        assert "central_diff" in methods
        assert "savitzky_golay" in methods
        assert len(methods) >= 3  # At least 3 methods
    ...
    def test_apply_derivative_operation(self, qtbot):
        ...
        dialog.derivative_method.setCurrentText("central_diff")
        ...
        assert params["method"] == "central_diff"
    ...
    def test_apply_resample_operation(self, qtbot):
        ...
        # Resample operation currently shares the dialog interface
        # and may not have specific widgets; skip if not implemented
        dialog = MathAnalysisDialog("resample")
        qtbot.addWidget(dialog)
        ...
        # Test that apply emits a signal even without specific resample widgets
        received_signals = []
        dialog.operation_requested.connect(lambda op, params: received_signals.append((op, params)))
        dialog._apply_operation()
